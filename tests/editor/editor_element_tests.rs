...
mod tests {
    ...
    fn test_editor_element_creation() {
        ...
        assert_eq!(settings.get_f32(vec!["ui", "editor", "line_height_px"]).unwrap(), 19.2);
    }
    ...
    fn test_editor_element_all_settings_accessible() {
        ...
        assert!(settings.get_f32(vec!["ui", "editor", "line_height_px"]).is_ok());
        assert!(settings.get_f32(vec!["ui", "editor", "char_spacing_px"]).is_ok());
        assert!(settings.get_color(vec!["ui", "editor", "cursor_color"]).is_ok());
        assert!(settings.get_f32(vec!["ui", "panels", "status_bar", "height_px"]).is_ok());
    }
