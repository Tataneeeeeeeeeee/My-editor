...
impl EditorElement {
    ...
    fn render_gutter(&self) -> impl IntoElement {
        ...
        let line_height = self
            ...
            .get_f32(vec!["ui", "editor", "line_height_px"])
            .unwrap_or(19.2);
        let gutter_text_color = self
            .settings_global
            .get_color(vec!["ui", "editor", "gutter", "text_color"])
            .unwrap_or(0x858585);
        ...
        div()
            ...
            .child(
                ...
                div().h(px(start_line as f32 * line_height)),
            )
            .children((start_line..end_line).map(|i| div().child(format!("{:>4} ", i + 1))))
    }

    fn render_text_with_cursor(&self) -> impl IntoElement {
        let char_spacing = self
            .settings_global
        ...
        for (idx, line) in lines.iter().enumerate() {
            let styled_segments = if line.is_empty() {
                ...
                let mut segments = self.syntax_highlighter.highlight_line(
                    &line_with_newline,
                    syntax,
                    &mut highlighter,
                );
        ...
        div()
            ...
            .children(
                visible_lines_with_highlighting
                    .into_iter()
                    .map(|styled_segments| {
                        div()
                            .flex()
                            ...
                            .children(if styled_segments.is_empty() {
                                // Empty line
                                vec![div().child(" ")]
                            } else {
    ...
    fn render_cursor(&self) -> impl IntoElement {
        ...
        let cursor_color = self
            ...
            .unwrap_or(0xffffff);
        let cursor_opacity = self
            .settings_global
            .get_f32(vec!["ui", "editor", "cursor_opacity"])
            .unwrap_or(0.9);
        let cursor_width = self
            .settings_global
            .get_f32(vec!["ui", "editor", "cursor_width_px"])
            .unwrap_or(2.0);
        ...
        div().absolute().left_0().top(px(cursor_y)).child(
            div()
                .flex()
                .flex_row()
                .text_size(px(16.0))
    ...
    fn render_status_bar(&self) -> impl IntoElement {
        let text_color = self
            .settings_global
            .get_color(vec!["ui", "panels", "status_bar", "text_color"])
            .unwrap_or(0x858585);
        let border_color = self
            .settings_global
            .get_color(vec!["ui", "panels", "status_bar", "border_color"])
            .unwrap_or(0x404040);
        ...
        div()
            ...
            .child(div().child(
                match self.settings_global.get(vec![
                    "file_extensions",
                    &self.file_extension,
                    "name",
                ]) {
    ...
    fn render_editor_content(&self) -> impl IntoElement {
        let bg_color = self
            .settings_global
            .get_color(vec!["ui", "editor", "background"])
            .unwrap_or(0x1e1e1e);
    ...
    fn render_scrollbar(&self) -> impl IntoElement {
        ...
        let scrollbar_width = self
            ...
            .unwrap_or(14.0);
        let scrollbar_bg = self
            .settings_global
            .get_color(vec!["ui", "editor", "scrollbar", "background"])
            .unwrap_or(0x1e1e1e);
        let scrollbar_track_bg = self
            .settings_global
            .get_color(vec!["ui", "colors", "background"])
            .unwrap_or(0x2d2d30);
        let thumb_color = self
            .settings_global
            .get_color(vec!["ui", "editor", "scrollbar", "thumb_color"])
            .unwrap_or(0x424242);
        let thumb_hover_color = self
            .settings_global
            .get_color(vec!["ui", "editor", "scrollbar", "thumb_hover_color"])
            .unwrap_or(0x4f4f4f);
        ...
        let thumb_ratio =
            (self.viewport_height / content_height.max(self.viewport_height)).min(1.0);
