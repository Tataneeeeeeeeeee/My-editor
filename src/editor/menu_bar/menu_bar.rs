...
impl MenuBar {
    ...
    pub fn render(&self, _file_menu_open: bool, cx: &mut Context<crate::editor::editor_window::EditorWindow>) -> impl IntoElement + use<> {
        ...
        let menu_bar_bg = settings_global.get_color(vec!["ui", "panels", "menu_bar", "background"]).unwrap_or(0x2d2d30);
        let menu_bar_border = settings_global.get_color(vec!["ui", "panels", "menu_bar", "border_color"]).unwrap_or(0x1e1e1e);
        let menu_text = settings_global.get_color(vec!["ui", "colors", "text_secondary"]).unwrap_or(0xcccccc);
        let menu_hover_bg = settings_global.get_color(vec!["ui", "hover_effects", "menu_hover_background"]).unwrap_or(0x3e3e42);
