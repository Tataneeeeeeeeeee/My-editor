...
impl TextBuffer {
    ...
    pub fn update_stats(&mut self) {
        ...
        let lines: Vec<&str> = self.text.lines().collect();
        let mut line_count = if self.text.is_empty() {
            1
        } else {
            lines.len().max(1)
        };
        ...
        let text_before_cursor = &self.text[..safe_cursor];
        let mut current_line = if text_before_cursor.is_empty() {
            1
        } else {
            text_before_cursor.lines().count()
        };
        let current_col = text_before_cursor
            .chars()
            .rev()
            .take_while(|&c| c != '\n')
            .count();
